...
KERNEL_CONFIG_MERGE := ARCH=$(KERNEL_ARCH) $(KERNEL_MERGE_CONFIG) -m -O $(TARGET_KERNEL_SOURCE)
KERNEL_DEFAULT_BUILD_CMD := $(MAKE) -C $(TARGET_KERNEL_SOURCE) O=$(PWD)/$(KERNEL_OBJECTS) ARCH=$(KERNEL_ARCH) CROSS_COMPILE=$(KERNEL_CROSS_COMPILE) KCONFIG_ALLCONFIG=.config alldefconfig
KERNEL_DISTCLEAN := $(MAKE) -C $(TARGET_KERNEL_SOURCE) distclean
...
$(error TARGET_BUILD_VARIANT was not defined, cannot make defconfig)
endif
MAKE_CONFIG_CMD := $(KERNEL_CONFIG_MERGE) $(KERNEL_DEFCONFIG_BASE) $(KERNEL_DEFCONFIG_PATH) $(KERNEL_USER_CFG);
MAKE_CONFIG_CMD += $(KERNEL_DEFAULT_BUILD_CMD);
MAKE_CONFIG_CMD += $(KERNEL_DISTCLEAN)
