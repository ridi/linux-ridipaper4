...
extern void increase_epd_fb_count(void);
#else
//for lt9211 init parameters
extern void lt9211_init(void);
extern void lt9611_init_ext(void);
extern bool lt9611_get_hpd_status_ext(void);
//extern void sys1p_gpio_down(void);
#endif
...
extern int dsim_registe_dpu_sysmmu_handler(void);
static disable_bootloader_logo = 0;
...
static int decon_enable(struct decon_device *decon)
{
	...
	if (decon->state == next_state) {
        ...
        if (disable_bootloader_logo == 0) {
                disable_bootloader_logo = 1;
                decon_stop();
                ret = _decon_disable(decon, DECON_STATE_OFF);
	        if (ret < 0) {
		        decon_err("decon-%d failed to set %s (ret %d)\n",
				decon->id, DECON_STATE_OFF, ret);
		        goto out;
	        }
                ret = dsim_registe_dpu_sysmmu_handler();
                if (ret) {
                        decon_err("%s:register  sysmmu handler error\n", __func__);
		        goto out;
                }
        }
...
#if !defined(CONFIG_MACH_RP400)
		//if (lt9611_get_hpd_status_ext() == 0)//hdmi  is plug-in do not power down lt9211/lt9611 power
		//sys1p_gpio_down();
#endif
		break;
