...
static struct dsim_device *_dsim;
...
#if !defined(CONFIG_EXYNOS_VIRTUAL_DISPLAY)
...
int dsim_registe_dpu_sysmmu_handler(void)
{
        int ret = 0;
...
#if defined(CONFIG_MACH_RP400)
	_dsim = dsim;
	dsim->vdd085 = devm_regulator_get(dev, "vdd085");
	if (IS_ERR(dsim->vdd085)) {
		ret = PTR_ERR(dsim->vdd085);
		if (ret != -EPROBE_DEFER)
			dev_err(dev, "Failed to get 'vdd085' regulator: %d\n", ret);
		return ret;
	}

	dsim->vdd18 = devm_regulator_get(dev, "vdd18");
	if (IS_ERR(dsim->vdd18)) {
		ret = PTR_ERR(dsim->vdd18);
		if (ret != -EPROBE_DEFER)
			dev_err(dev, "Failed to get 'vdd18' regulator: %d\n", ret);
		return ret;
	}

	regulator_enable(dsim->vdd18);
	regulator_enable(dsim->vdd085);
#endif
...
#if 0//remove register iovmm_handler
...
#if defined(CONFIG_MACH_RP400)
static int dsim_suspend_late(struct device *dev)
{
	struct dsim_device *dsim = dev_get_drvdata(dev);

	dsim_info("%s +\n", __func__);
	regulator_disable(dsim->vdd18);
	regulator_disable(dsim->vdd085);
	dsim_info("%s -\n", __func__);

	return 0;
}

static int dsim_resume_early(struct device *dev)
{
	struct dsim_device *dsim = dev_get_drvdata(dev);
...
void dsim_powerup(void)
{
	if(power_down) {
		power_down = 0;
		regulator_enable(_dsim->vdd18);
		regulator_enable(_dsim->vdd085);
		dsim_err("%s\n", __func__);
	}
}

void dsim_powerdown(void)
{
	if(!power_down) {
		power_down = 1;
		disable_irq(_dsim->res.irq);
		regulator_disable(_dsim->vdd18);
		regulator_disable(_dsim->vdd085);
		dsim_err("%s\n", __func__);
	}
}

int dsim_power_status(void)
{
	return !power_down;
}

int dsim_status(void)
{
	return (_dsim)?(int)_dsim->state:0;
}
#endif
...
static const struct dev_pm_ops dsim_pm_ops = {
	...
	.suspend_late		= dsim_suspend_late,
	.resume_early		= dsim_resume_early,
};
