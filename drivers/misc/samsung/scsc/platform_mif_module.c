...
#if defined(CONFIG_MACH_RP400)
	mif_module.vdd08 = devm_regulator_get(dev, "vdd08");
	if (IS_ERR(mif_module.vdd08)) {
		ret = PTR_ERR(mif_module.vdd08);
		if (ret != -EPROBE_DEFER)
			dev_err(dev, "Failed to get 'vdd08' regulator: %d\n", ret);
		return ret;
	}

	mif_module.vdd12 = devm_regulator_get(dev, "vdd12");
	if (IS_ERR(mif_module.vdd12)) {
		ret = PTR_ERR(mif_module.vdd12);
		if (ret != -EPROBE_DEFER)
			dev_err(dev, "Failed to get 'vdd12' regulator: %d\n", ret);
		return ret;
	}

	mif_module.vdd18 = devm_regulator_get(dev, "vdd18");
	if (IS_ERR(mif_module.vdd18)) {
		ret = PTR_ERR(mif_module.vdd18);
		if (ret != -EPROBE_DEFER)
			dev_err(dev, "Failed to get 'vdd18' regulator: %d\n", ret);
		return ret;
	}

	regulator_enable(mif_module.vdd08);
	regulator_enable(mif_module.vdd12);
	regulator_enable(mif_module.vdd18);
#endif
...
static int platform_mif_module_suspend(struct device *dev)
{
	...
	regulator_disable(mif_module.vdd08);
	regulator_disable(mif_module.vdd12);
	regulator_disable(mif_module.vdd18);
#endif
