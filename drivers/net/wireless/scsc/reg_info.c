...
int slsi_read_regulatory(struct slsi_dev *sdev)
{
	...
	uint32_t script_version, minor_version;
	...
	kernel_read(fptr, &script_version, sizeof(uint32_t), &fptr->f_pos);
	kernel_read(fptr, &sdev->regdb.version, sizeof(uint32_t), &fptr->f_pos);
	kernel_read(fptr, &minor_version, sizeof(uint32_t), &fptr->f_pos);
