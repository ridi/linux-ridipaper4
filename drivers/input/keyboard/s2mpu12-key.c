...
static struct input_dev *_input;
#endif
...
#if defined(CONFIG_MACH_RP400)
extern int is_operating_mtp(void);
extern int get_rp400_powerkey_lock(void);

void send_power_key_event(void)
{
	pr_err("%s\n", __func__);
	if(!_input) return;
	input_event(_input, EV_KEY, KEY_POWER, 1);
	input_sync(_input);
	input_event(_input, EV_KEY, KEY_POWER, 0);
	input_sync(_input);
}

void send_sleep_key_event(void)
{
	pr_err("%s\n", __func__);
	if(!_input) return;
	input_event(_input, EV_KEY, KEY_SLEEP, 1);
	input_sync(_input);
	input_event(_input, EV_KEY, KEY_SLEEP, 0);
	input_sync(_input);
}

void send_wakeup_key_event(void)
{
	pr_err("%s\n", __func__);
	if(!_input) return;
	input_event(_input, EV_KEY, KEY_WAKEUP, 1);
	input_sync(_input);
	input_event(_input, EV_KEY, KEY_WAKEUP, 0);
	input_sync(_input);
}
#endif
...
#if defined(CONFIG_MACH_RP400)
	if(is_operating_mtp()) {
		dev_err(ddata->dev, "power key is locking!\n");
		return;
	}

	if(get_rp400_powerkey_lock()) {
		if((key_locked == 0) && !state)
			goto exit;
		dev_err(ddata->dev, "power key is locking!\n");
		key_locked = 1;
		return;
...
{
#if defined(CONFIG_MACH_RP400)
	pr_err("Last power key is always released.");
	return 0;
#else
