...
static int elants_i2c_query_fw_version(struct elants_data *ts)
{
	...
	for (retry_cnt = 0; retry_cnt < MAX_RETRIES; retry_cnt++) {
		...
		dev_dbg(&client->dev, "[elan] %s: (Firmware version) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, resp[0], resp[1], resp[2], resp[3] , resp[4], resp[5], resp[6], resp[7]);
...
static int elants_i2c_query_test_version(struct elants_data *ts)
{
	...
	for (retry_cnt = 0; retry_cnt < MAX_RETRIES; retry_cnt++) {
		error = elants_i2c_execute_command(client, cmd, sizeof(cmd),
                                           resp, sizeof(resp));
		dev_dbg(&client->dev, "[elan] %s: (test version) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, resp[0], resp[1], resp[2], resp[3] , resp[4], resp[5], resp[6], resp[7]);
...
static int elants_i2c_query_bc_version(struct elants_data *ts)
{
	...
	error = elants_i2c_execute_command(client, cmd, sizeof(cmd),
                                       resp, sizeof(resp));
	dev_dbg(&client->dev, "[elan] %s: (BC version) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, resp[0], resp[1], resp[2], resp[3] , resp[4], resp[5], resp[6], resp[7]);
...
static int elants_i2c_query_ts_info(struct elants_data *ts)
{
	...
	*/
	...
	dev_dbg(&client->dev, "[elan] %s: (get resolution) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, resp[0], resp[1], resp[2], resp[3] , resp[4], resp[5], resp[6], resp[7]);
	/*  
	...
	*/
	...
	dev_dbg(&client->dev, "[elan] %s: (get osr) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, resp[0], resp[1], resp[2], resp[3] , resp[4], resp[5], resp[6], resp[7]);
	//osr = resp[3];
	osr = resp[7];
	...
	dev_dbg(&client->dev, "[elan] %s: (get physical scan) %2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", __func__, resp[0], resp[1], resp[2], resp[3] , resp[4], resp[5], resp[6], resp[7]);
                                                 ...
                                                 const struct firmware *fw)
{
 ...
 IAP_RESTART:
	...
	for (iPage = 1; iPage <= PageNum; iPage +=FlashPageNum) {
		...
		if (res != ACK_OK) {
			if (retry_num < 1) {
				dev_err(&ts->client->dev, "[elan] Update Firmware retry_num=%d fail!!(res=0x%x\n",
						retry_num, res);
				retry_num++;
				curIndex = 0;
				goto IAP_RESTART;
				...
				dev_err(&ts->client->dev,
						"[elan] Update Firmware retry_num=%d!!(res=0x%x\n",
						retry_num, res);
				goto IAP_END;
			}
		}
                                         ...
                                         bool force)
{
	...
	if (force) {
		...
		if (check_remark_id) {
			error = elants_i2c_validate_remark_id(ts, fw->data, fw->size);
			if (error)
				return error;
		}
...
static int elants_i2c_fw_update(struct elants_data *ts)
{
	...
	fp = filp_open(ts->fw_path, O_RDONLY, S_IRUSR/*|S_IWUSR*/);
	if(IS_ERR(fp)) {
		dev_err(&client->dev, "%s file open failed!\n", ts->fw_path);
		set_fs(old_fs);
		return -ENOENT;
	}
	...
	fw->size = 0;
	fw->data = kmalloc(256*1024, GFP_KERNEL);
	do {
		error = vfs_read(fp, (char *)&fw->data[fw->size], ELAN_FW_PAGESIZE, &fp->f_pos);
		fw->size += error;
	} while(error != 0);
...
static void elants_auto_update(struct work_struct *work)
{
	...
	dev_info(&client->dev, "[elan]entery elants_auto_update\n");
/*
...
*/
	...
	if (error == 0x56) {
		...
		new_hw_version  = fw_data[0x1FE67] << 8  | fw_data[0x1FE66]; //addr should be modified depends on different project
    ...
    for(i = 0; i < finger_num; i++) {
		...
		if(active) {
			...
			*/
            ...
            input_report_abs(idev, ABS_MT_POSITION_X, ts->x_max-x);
	...
	case ELAN_STATE_NORMAL:
		...
		case CMD_HEADER_HID_I2C:  //0x43
			...
			dev_info(&client->dev, "[elan] %2x,%2x,%2x,%2x,%2x,%2x,%2x,%2x,%2x,%2x\n", 
                     ts->buf[0], ts->buf[1], ts->buf[2], ts->buf[3], ts->buf[4], ts->buf[5], ts->buf[6], ts->buf[7], ts->buf[8], ts->buf[9]);
		...
		const char *buf, size_t count)
{
	...
	dev_err(&client->dev, "Touch sleep %d\n", sleep);
...
static DEVICE_ATTR(sleep, S_IWUSR, NULL, sleep_store);
...
#endif
	dev_info(&client->dev, "[elan] enter elants_i2c_probe...v%s\n", DRV_VERSION);
