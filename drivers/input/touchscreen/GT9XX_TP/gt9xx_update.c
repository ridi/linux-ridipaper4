...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
	...
	if (cfg->length < GTP_CONFIG_MIN_LENGTH) {
		dev_err(&ts->client->dev,
			"No valid config with sensor_ID(%d) ", sensor_id);
...
static u8 gup_get_ic_msg(struct i2c_client *client, u16 addr, u8 * msg, s32 len)
{
	...
	if (i >= 5) {
		dev_err(&client->dev,
			"Read data from 0x%02x%02x failed!", msg[0], msg[1]);
		return FAIL;
	}
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
	...
	for (retry = 0; retry < 2; retry++) {
		...
		if ((0xBE != update_msg.fw_damaged) && (!retry)) {
			...
			dev_info(&client->dev,
				 "The IC will be updated by force.");
			continue;
		}
		break;
	}
			...
			u16 start_addr, u16 total_length)
{
	...
	while (burn_length < total_length) {
		...
		frame_length = ((total_length - burn_length)
				> PACK_SIZE) ? PACK_SIZE : (total_length -
							    burn_length);
			   ...
			   u16 start_rd_addr, u16 chk_length)
{
	...
	while (recall_length < chk_length) {
		...
		if (memcmp(&rd_buf[GTP_ADDR_LENGTH],
			...
			dev_err(&client->dev,
				"Recall frame data fail,not equal.");
	...
	dev_dbg(&client->dev,
		"Recall check %dk firmware success.", (chk_length / 1024));
...
static u8 gup_burn_fw_gwake_section(struct i2c_client *client,
				    u8 * fw_section, u16 start_addr, u32 len,
				    u8 bank_cmd)
{
	s32 ret = 0;
	u8 rd_buf[5];
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
	...
	ret = gup_burn_fw_gwake_section(client,
					fw_gwake, 0x9000, FW_SECTION_LENGTH,
					0x3A);
	if (FAIL == ret) {
		dev_err(&client->dev,
