...
// #define dev_dbg(dev, fmt, args...) pr_info("[%s]<%s:%d>: " fmt, MY_TAG, __func__, __LINE__, ##args)
...
int gtp_i2c_write(struct i2c_client *client, u8 * buf, int len)
{
	...
	while (pos != len) {
		if (unlikely
		    (len - pos > I2C_MAX_TRANSFER_SIZE - GTP_ADDR_LENGTH))
			transfer_length =
			 ...
			 struct goodix_point_t *points, u8 * key_value)
{
	...
	*key_value = point_data[3 + 8 * touch_num];
	...
	for (i = 0; i < touch_num; i++) {
		...
		//printk("20200813 zengxl:x=%d y=%d swap=%d convertx=%d converty=%d\n",ts->pdata->abs_size_x,ts->pdata->abs_size_y,ts->pdata->swap_x2y,ts->pdata->convertx,ts->pdata->converty);
		if (ts->pdata->convertx)
			points[i].x = ts->pdata->abs_size_x - points[i].x;
		if (ts->pdata->converty)
			points[i].y = ts->pdata->abs_size_y - points[i].y;
		if (ts->pdata->swap_x2y)
			GTP_SWAP(points[i].x, points[i].y);

		//weifm: print touch data
		...
		if (points[i].id & 0x80) {
			...
			if (ts->pdata->pen_suppress_finger) {
				points[0] = points[i];
				memset(++points, 0,
				       sizeof(*points) * (GTP_MAX_TOUCH_ID -
							  1));
				finger_state &= 0xf0;
				finger_state |= 0x01;
				break;
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	gtp_int_output(ts, client->addr == 0x40);
			...
			struct goodix_ts_platform_data *pdata)
{
	...
	of_property_read_u32(np, "goodix,convertx", &pdata->convertx);
	if (pdata->convertx)
		dev_info(dev, "convertx enabled\n");

	of_property_read_u32(np, "goodix,converty", &pdata->converty);
	if (pdata->converty)
		dev_info(dev, "converty enabled\n");
...
static int gtp_power_on(struct goodix_ts_data *ts)
{
	...
	if (ts->avdd_3v3) {
		ret = regulator_enable(ts->avdd_3v3);
		if (ret) {
			dev_err(&ts->client->dev,
				"Regulator avdd_3v3 enable failed ret=%d\n",
				ret);
...
static int gtp_power_init(struct goodix_ts_data *ts)
{
	...
	ts->avdd_3v3 = regulator_get(&ts->client->dev, "vdd_ldo27");
	if (IS_ERR(ts->avdd_3v3)) {
		ts->vdd_ana = NULL;
		ret = PTR_ERR(ts->avdd_3v3);
		dev_info(&ts->client->dev,
			 "Regulator get failed avdd_3v3 ret=%d\n", ret);
	}
...
static int gtp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "I2C Addr is 0x%x\n", client->addr);
...
static const struct dev_pm_ops gtp_pm_ops = {.suspend = gtp_pm_suspend,
...
static int gtp_register_powermanager(struct goodix_ts_data *ts)
{
	...
	ts->early_suspend.resume = goodix_ts_alte_resume;
