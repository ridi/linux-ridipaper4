...
void init_clodreset_mutex(void)
{
...
static int sec_nfc_resume(struct device *dev)
{
    ...
    if (info->pinctrl != NULL) {
        pinctrl_select_state(info->pinctrl, info->resume);
    }
...
static int __sec_nfc_probe(struct device *dev)
{
    ...
    info->defaul = pinctrl_lookup_state(info->pinctrl, "default");
    if (IS_ERR(info->defaul)) {
        pr_err("%s: failed to lookup default state \n", __func__);
        info->defaul = NULL;
        goto err_pinctrl;
    }
    info->resume = pinctrl_lookup_state(info->pinctrl, "resume");
    if (IS_ERR(info->resume)) {
        pr_err("%s: failed to lookup resume state \n", __func__);
        info->resume = NULL;
        goto err_pinctrl;
    }
    info->sleep = pinctrl_lookup_state(info->pinctrl, "sleep");
    if (IS_ERR(info->sleep)) {
        pr_err("%s: failed to lookup sleep state \n", __func__);
        info->sleep = NULL;
        goto err_pinctrl;
    }

    if (info->pinctrl != NULL) {
        pinctrl_select_state(info->pinctrl, info->defaul);
        pinctrl_select_state(info->pinctrl, info->resume);
    }
