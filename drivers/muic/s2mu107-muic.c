...
#if IS_ENABLED(CONFIG_S2MU107_TYPEC_WATER)
#include <linux/fb.h>
...
#if IS_ENABLED(CONFIG_S2MU107_TYPEC_WATER)
static void s2mu107_muic_set_water_state(struct s2mu107_muic_data *muic_data, bool en);
static void s2mu107_muic_set_rid_for_water(struct s2mu107_muic_data *muic_data, bool en);
static void s2mu107_muic_set_rid_int_mask_en(struct s2mu107_muic_data *muic_data, bool en);
#if IS_ENABLED(CONFIG_HICCUP_CHARGER)
...
static int s2mu107_muic_detect_usb_killer(struct s2mu107_muic_data *muic_data)
{
	...
	reg_val = s2mu107_i2c_read_byte(i2c, S2MU107_REG_AFC_STATUS);
	pr_info("%s(%d) reg_val(%#x)\n", __func__, __LINE__, reg_val);
	dnres = reg_val & AFC_CTRL2_DNRESEN_MASK;
	if (!dnres) {
...
exit_chk:
	...
	if (muic_data->water_status != S2MU107_WATER_MUIC_IDLE)
...
#if IS_ENABLED(CONFIG_S2MU107_TYPEC_WATER)
static void s2mu107_if_set_water_det(void *mdata, bool val)
{
	struct s2mu107_muic_data *muic_data =
		(struct s2mu107_muic_data *)mdata;
	...
	if (muic_data->water_dry_status == S2MU107_WATER_DRY_MUIC_DET) {
		if (val)
			muic_data->water_dry_status = S2MU107_WATER_DRY_MUIC_CCIC_INVALID;
		else
			muic_data->water_dry_status = S2MU107_WATER_DRY_MUIC_CCIC_DET;
		wake_up_interruptible(&muic_data->wait);
	} else {
		if (muic_data->water_status >= S2MU107_WATER_MUIC_DET) {
			if (val)
				muic_data->water_status = S2MU107_WATER_MUIC_CCIC_DET;
			else
				muic_data->water_status = S2MU107_WATER_MUIC_CCIC_INVALID;
			pr_info("%s water_status : (%d)\n",
				__func__, muic_data->water_status);
			wake_up_interruptible(&muic_data->wait);
		} else {
...
#if !IS_ENABLED(CONFIG_SEC_FACTORY)
static void s2mu107_if_set_water_det_from_boot(void *mdata, bool val)
{
	struct s2mu107_muic_data *muic_data =
		(struct s2mu107_muic_data *)mdata;

	if (val)
		s2mu107_muic_set_water_state(muic_data, MUIC_ENABLE);
}
#endif
#endif
...
static int s2mu107_muic_reg_init(struct s2mu107_muic_data *muic_data)
{
	...
	s2mu107_muic_set_rid_for_water(muic_data, MUIC_ENABLE);
#endif
...
#if IS_ENABLED(CONFIG_S2MU107_TYPEC_WATER)
static void s2mu107_muic_handle_legacy_detach(struct s2mu107_muic_data *muic_data)
{
	muic_data->rescan_cnt = 0;
	s2mu107_muic_set_rid_int_mask_en(muic_data, MUIC_DISABLE);
	return;
}
#endif
...
#if IS_ENABLED(CONFIG_S2MU107_TYPEC_WATER)
			s2mu107_muic_handle_legacy_detach(muic_data);
#endif
		}
#endif
	} else {
...
static void s2mu107_muic_set_rid_int_mask_en(struct s2mu107_muic_data *muic_data,
		bool en)
{
	struct i2c_client *i2c = muic_data->i2c;
	...
	pr_info("%s en : %d\n",	__func__, (int)en);
	if (en) {
		s2mu107_i2c_write_byte(i2c, S2MU107_REG_MUIC_INT1_MASK, INT_WATER_MASK1);
		s2mu107_i2c_write_byte(i2c, S2MU107_REG_MUIC_INT2_MASK, INT_WATER_MASK2);
	} else {
		s2mu107_i2c_write_byte(i2c, S2MU107_REG_MUIC_INT1_MASK, INT_MUIC_MASK1);
		s2mu107_i2c_write_byte(i2c, S2MU107_REG_MUIC_INT2_MASK, INT_MUIC_MASK2);
	}
...
static void s2mu107_muic_set_rid_for_water(struct s2mu107_muic_data *muic_data,
		bool en)
{
#if TEMP_BRINGUP
	struct i2c_client *i2c = muic_data->i2c;
	...
	reg_val = s2mu107_i2c_read_byte(i2c, S2MU107_REG_ADCBIAS_OTP4);
	if (en) {
		reg_val &= ~(ADCBIAS_OTP4_ADC_STDBY_BIAS_SEL_MASK);
	} else {
		reg_val |= (ADCBIAS_OTP4_ADC_STDBY_BIAS_SEL_MASK);
	}
	s2mu107_i2c_write_byte(i2c, S2MU107_REG_ADCBIAS_OTP4, reg_val);
	...
	i2c = muic_data->i2c;
	reg_rid_ldo = s2mu107_i2c_read_byte(i2c, S2MU107_REG_ADCBIAS_OTP4);
	reg_std_bias = s2mu107_i2c_read_byte(i2c, S2MU107_REG_RID_WATER_PROOF);
	...
	if (en) {
		reg_rid_ldo |= ADCBIAS_OTP4_ADC_STDBY_BIAS_SEL_MASK;
		reg_std_bias |= 0xF0;
		s2mu107_i2c_write_byte(i2c, S2MU107_REG_RID_WATER_PROOF, reg_std_bias);
	} else {
		reg_rid_ldo &= ~ADCBIAS_OTP4_ADC_STDBY_BIAS_SEL_MASK;
	}
#endif
	s2mu107_i2c_write_byte(i2c, S2MU107_REG_ADCBIAS_OTP4, reg_rid_ldo);
#endif
}

static void s2mu107_muic_put_dry_chk_time(struct s2mu107_muic_data *muic_data)
{
	struct timeval time;
...
static int s2mu107_muic_check_water_adc(struct s2mu107_muic_data *muic_data)
{
	int i, adc = 0;
	...
	for (i = 0; i < WATER_DET_RETRY_CNT; i++) {
		adc = _s2mu107_muic_recheck_adc(muic_data);
		if (!IS_WATER_ADC(adc)) {
			pr_info("%s : NOT WATER\n", __func__);
			return adc;
		}

		if (_s2mu107_muic_get_vbus_state(muic_data)) {
			pr_info("%s : vbus while detecting\n", __func__);
			return adc;
		}

		pr_info("%s : %dst try : adc(0x%x)\n", __func__, i, adc);
	}

	return adc;
}

static void s2mu107_muic_set_water_state(struct s2mu107_muic_data *muic_data,
		bool en)
{
#if IS_ENABLED(CONFIG_VBUS_NOTIFIER)
	int vbvolt = 0;
	...
	if (en) {
		...
		muic_data->water_status = S2MU107_WATER_MUIC_CCIC_STABLE;
		muic_data->water_dry_status = S2MU107_WATER_DRY_MUIC_IDLE;
#if IS_ENABLED(CONFIG_VBUS_NOTIFIER)
		vbvolt = _s2mu107_muic_get_vbus_state(muic_data);
		vbus_notifier_handle(vbvolt ? STATUS_VBUS_HIGH : STATUS_VBUS_LOW);
#endif /* CONFIG_VBUS_NOTIFIER */
		s2mu107_muic_put_dry_chk_time(muic_data);
		...
		muic_data->water_status = S2MU107_WATER_MUIC_IDLE;
		muic_data->water_dry_status = S2MU107_WATER_DRY_MUIC_IDLE;
#if IS_ENABLED(CONFIG_HICCUP_CHARGER)
		s2mu107_muic_set_hiccup_mode(muic_data, MUIC_DISABLE);
#endif
		muic_data->dry_duration_sec = WATER_DRY_RETRY_INTERVAL_SEC;
		muic_data->dry_cnt = 0;
		s2mu107_muic_set_rid_for_water(muic_data, MUIC_ENABLE);
...
static void s2mu107_muic_water_detect_handler(struct work_struct *work)
{
	struct s2mu107_muic_data *muic_data =
		container_of(work, struct s2mu107_muic_data, water_detect_handler.work);
	int wait_ret = 0;

	mutex_lock(&muic_data->water_det_mutex);
	wake_lock(&muic_data->water_wake_lock);
	if (muic_data->water_status != S2MU107_WATER_MUIC_VERIFY) {
	...
	muic_data->water_status = S2MU107_WATER_MUIC_DET;
	_s2mu107_muic_control_rid_adc(muic_data, MUIC_DISABLE);
	...
	if ((wait_ret < 0) || (!wait_ret)) {
		...
		if (muic_data->water_status == S2MU107_WATER_MUIC_CCIC_DET) {
			s2mu107_muic_set_water_state(muic_data, MUIC_ENABLE);
		} else if (muic_data->water_status == S2MU107_WATER_MUIC_CCIC_INVALID) {
...
static void s2mu107_muic_water_dry_handler(struct work_struct *work)
{
	struct s2mu107_muic_data *muic_data =
		container_of(work, struct s2mu107_muic_data, water_dry_handler.work);
	int adc, i = 0;
	int wait_ret = 0;
	...
	if (muic_data->water_status != S2MU107_WATER_MUIC_CCIC_STABLE) {
	...
	s2mu107_muic_put_dry_chk_time(muic_data);
	s2mu107_muic_set_rid_int_mask_en(muic_data, true);
	...
	s2mu107_muic_set_rid_int_mask_en(muic_data, false);
	muic_data->water_dry_status = S2MU107_WATER_DRY_MUIC_DET;
	...
	if ((wait_ret < 0) || (!wait_ret)
		...
		schedule_delayed_work(&muic_data->water_dry_handler,
			msecs_to_jiffies(WATER_DRY_RETRY_INTERVAL_MS));
	} else if (muic_data->water_dry_status == S2MU107_WATER_DRY_MUIC_CCIC_DET) {
		msleep(500);
		s2mu107_muic_set_water_state(muic_data, MUIC_DISABLE);
	}
...
static void s2mu107_muic_sleep_dry_checker(struct work_struct *work)
{
	struct s2mu107_muic_data *muic_data =
		container_of(work, struct s2mu107_muic_data, sleep_dry_checker.work);
	struct timeval time;
	long duration;

	pr_info("%s entered\n", __func__);
	if (muic_data->water_status == S2MU107_WATER_MUIC_CCIC_STABLE && muic_data->lcd_on) {
		if (!_s2mu107_muic_get_vbus_state(muic_data)) {
			do_gettimeofday(&time);
...
static void s2mu107_muic_water_judge(struct s2mu107_muic_data *muic_data)
{
	pr_info("%s water_status(%d)\n", __func__, muic_data->water_status);

	if (muic_data->water_status == S2MU107_WATER_MUIC_IDLE) {
		/* Set LPM */
		MUIC_SEND_NOTI_TO_CCIC_DETACH(ATTACHED_DEV_TYPE3_MUIC);
		muic_data->water_status = S2MU107_WATER_MUIC_VERIFY;
		msleep(100);
		muic_data->adc = s2mu107_muic_check_water_adc(muic_data);
		muic_data->vbvolt = _s2mu107_muic_get_vbus_state(muic_data);
		/* Set Normal */
		...
		unsigned long val, void *v)
{
	...
	struct s2mu107_muic_data *muic_data =
		container_of(this, struct s2mu107_muic_data, fb_notifier);
	int fb_blank = 0;
...
static irqreturn_t s2mu107_muic_attach_isr(int irq, void *data)
{
	...
	if (s2mu107_muic_is_opmode_typeC(muic_data)) {
		if (IS_WATER_ADC(muic_data->adc) || IS_WATER_STATUS(muic_data->water_status)) {
			goto attach_skip;
		}
	}
#endif
...
#if IS_ENABLED(CONFIG_S2MU107_TYPEC_WATER) && IS_ENABLED(CONFIG_HICCUP_CHARGER)
	if (!lpcharge && IS_WATER_STATUS(muic_data->water_status)) {
...
static irqreturn_t s2mu107_muic_adc_change_isr(int irq, void *data)
{
	...
	if (muic_if->opmode & OPMODE_MUIC) {
		...
		s2mu107_muic_water_judge(muic_data);
#endif
	}
...
#endif
#if IS_ENABLED(CONFIG_S2MU107_TYPEC_WATER)
	muic_if->set_water_detect = s2mu107_if_set_water_det;
#if !IS_ENABLED(CONFIG_SEC_FACTORY)
	muic_if->set_water_detect_from_boot = s2mu107_if_set_water_det_from_boot;
#endif
#endif
#if IS_ENABLED(CONFIG_HICCUP_CHARGER)
...
static int s2mu107_muic_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&muic_data->water_dry_handler,
		s2mu107_muic_water_dry_handler);
	INIT_DELAYED_WORK(&muic_data->water_detect_handler,
		s2mu107_muic_water_detect_handler);
	INIT_DELAYED_WORK(&muic_data->sleep_dry_checker,
		s2mu107_muic_sleep_dry_checker);
...
#if IS_ENABLED(CONFIG_S2MU107_TYPEC_WATER)
	s2mu107_muic_set_rid_for_water(muic_data, MUIC_DISABLE);
#endif
	_s2mu107_muic_set_chg_det(muic_data, MUIC_ENABLE);
