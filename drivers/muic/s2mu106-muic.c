...
struct s2mu106_muic_data *g_muic_data;
...
void s2mu106_muic_get_detect_info(struct s2mu106_muic_data *muic_data)
{
	...
	if(psy_bat) {
		...
		pr_err("Charging input voltage: %d\n", pm_chgin);
		if(pm_chgin > 4000) {
			val.intval = POWER_SUPPLY_TYPE_MAINS;
		} else {
			val.intval = POWER_SUPPLY_TYPE_BATTERY;
		}
		power_supply_set_property(psy_bat, POWER_SUPPLY_PROP_ONLINE, &val);
	}

	if(muic_data->reg[CHG_TYPE] == 0x00) {
		pr_err("USB Not Detected %d\n", muic_data->reg[CHG_TYPE]);
		set_rp400_usb_detect(0);
	} else 	if(muic_data->reg[CHG_TYPE] & 0x04) {
		pr_err("USB Detected %d\n", muic_data->reg[CHG_TYPE]);
		set_rp400_usb_detect(1);
		send_wakeup_key_event();
	} else {
		pr_err("Charger Detected %d\n", muic_data->reg[CHG_TYPE]);
		set_rp400_usb_detect(2);
	}
#endif
}
