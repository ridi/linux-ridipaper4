...
static struct iio_dev *_indio_dev;
static int st_accel_probe_done = 0;

int is_st_accel_probe_done(void)
{
	return st_accel_probe_done;
}
#endif
...
#if defined(CONFIG_MACH_RP400)
int st_accel_read_raw_data(int* x, int* y, int* z)
{
	int ret = 0;
	struct iio_chan_spec const *ch0 = &st_accel_12bit_channels[0];
	struct iio_chan_spec const *ch1 = &st_accel_12bit_channels[1];
	struct iio_chan_spec const *ch2 = &st_accel_12bit_channels[2];

	if(!_indio_dev)
		return -ENODEV;

	ret = st_sensors_read_info_raw(_indio_dev, ch0, x);
	if(ret < 0)
		return ret;
	ret = st_sensors_read_info_raw(_indio_dev, ch1, y);
	if(ret < 0)
		return ret;
	ret = st_sensors_read_info_raw(_indio_dev, ch2, z);
	if(ret < 0)
		return ret;
	return ret;
}
...
#if defined(CONFIG_MACH_RP400)
	_indio_dev = indio_dev;
	st_accel_probe_done = 1;
#endif
