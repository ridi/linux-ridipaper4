...
static void sensor_3l6_cis_data_calculation(const struct sensor_pll_info_compact *pll_info_compact, cis_shared_data *cis_data)
{
	u32 vt_pix_clk_hz = 0;
...
int sensor_3l6_cis_check_rev(struct v4l2_subdev *subdev)
{
	int ret = 0;
	u8 rev = 0;
...
int sensor_3l6_cis_long_term_exposure(struct v4l2_subdev *subdev)
{
	int ret = 0;
	struct is_cis *cis;
...
static int sensor_3l6_cis_set_dual_master_setting(struct is_cis *cis)
{
	int ret = 0;
	struct i2c_client *client;
...
int sensor_3l6_cis_set_dual_setting(struct v4l2_subdev *subdev, u32 mode)
{
	int ret = 0;
	struct is_cis *cis;
	...
	case DUAL_SYNC_MASTER:
		ret = sensor_3l6_cis_set_dual_master_setting(cis);
		if (ret)
			err("3l6 dual master setting fail");
		break;
...
static struct is_cis_ops cis_ops = {
	...
	.cis_check_rev_on_init = sensor_3l6_cis_check_rev,
	.cis_active_test = sensor_cis_active_test,
	.cis_set_long_term_exposure = sensor_3l6_cis_long_term_exposure,
	.cis_set_dual_setting = sensor_3l6_cis_set_dual_setting,
};
