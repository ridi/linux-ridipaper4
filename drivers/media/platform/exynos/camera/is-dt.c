...
int is_parse_dt(struct platform_device *pdev)
{
	...
	if (parse_gate_info(pdata, np) < 0)
		pr_isdt("can't parse clock gate info node");

	ret = of_property_read_u32(np, "chain_config", &core->chain_config);
	if (ret)
		pr_isdt("chain configuration read is fail(%d)", ret);
	...
	pr_isdt("ret(%d) secure_mem_info(%#08lx, %#08lx)", ret,
	...
	pr_isdt("ret(%d) non_secure_mem_info(%#08lx, %#08lx)", ret,
	...
	if (vender_np) {
		...
		if (ret)
			pr_isdt("is_vender_dt is fail(%d)", ret);
	}
