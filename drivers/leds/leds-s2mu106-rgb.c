...
//extern int get_epd_hw_test_mode(void);
extern int get_rp400_debug_mode(void);
#endif

static bool is_debug_mode(void)
{
#if defined(CONFIG_MACH_RP400)
	return ((get_rp400_debug_mode()&0x01)?true:false);
#else
	return true;
#endif
}

struct s2mu106_rgb_data *g_rgb_data = NULL;
...
#if defined(CONFIG_MACH_RP400)
	/*if(get_epd_hw_test_mode()) {
		return;
	}*/

	if (brightness == LED_OFF) {
		s2mu106_rgb_set_mode(rgb, 3, S2MU106_RGB_MODE_OFF);
	} else {
		s2mu106_rgb_set_curr(rgb, 3, brightness);
		s2mu106_rgb_set_mode(rgb, 3, S2MU106_RGB_MODE_CONST);
	}
#else
	if (brightness == LED_OFF)
...
#if defined(CONFIG_MACH_RP400)
	mode = s2mu106_rgb_get_mode(rgb, 3);
	if (mode == S2MU106_RGB_MODE_OFF)
		brightness = LED_OFF;
	else
		brightness = s2mu106_rgb_get_curr(rgb, 3);
#else
...
#if defined(CONFIG_MACH_RP400)
	//s2mu106_rgb_set_mode(g_rgb_data, 3, S2MU106_RGB_MODE_OFF);
#else
	s2mu106_rgb_set_mode(g_rgb_data, RED_RGB, S2MU106_RGB_MODE_OFF);

	s2mu106_rgb_set_mode(g_rgb_data, BLUE_RGB, S2MU106_RGB_MODE_CONST);
	s2mu106_rgb_set_curr(g_rgb_data, BLUE_RGB, 1);
#endif

	pr_info("%s: full_blue\n", __func__);
	return;
}
EXPORT_SYMBOL(poweroff_full_led);   //kernel extern use

void poweroff_notcharging_led(void)
{
	//if(!g_rgb_data){
	//	return;
	//}
...
#else
	//s2mu106_rgb_set_mode(g_rgb_data, RED_RGB, S2MU106_RGB_MODE_OFF);
	//s2mu106_rgb_set_mode(g_rgb_data, BLUE_RGB, S2MU106_RGB_MODE_OFF);
#endif

	//pr_info("%s: notcharging\n", __func__);
	return;
}
EXPORT_SYMBOL(poweroff_notcharging_led);   //kernel extern use

void poweroff_charging_led(void)
{
	if(!g_rgb_data){
		return;
	}
	...
	//s2mu106_rgb_set_mode(g_rgb_data, 3, S2MU106_RGB_MODE_CONST);
	//s2mu106_rgb_set_curr(g_rgb_data, 3, 100);
#else
	s2mu106_rgb_set_mode(g_rgb_data, BLUE_RGB, S2MU106_RGB_MODE_OFF);

	s2mu106_rgb_set_mode(g_rgb_data, RED_RGB, S2MU106_RGB_MODE_CONST);
	s2mu106_rgb_set_curr(g_rgb_data, RED_RGB, 1);
#endif

	if(is_debug_mode())
		pr_info("%s: charging_led\n", __func__);
	return;
}
EXPORT_SYMBOL(poweroff_charging_led);   //kernel extern use

#if defined(CONFIG_MACH_RP400)
void hw_test_charging_led(int on)
{
	if(on) {
		s2mu106_rgb_set_mode(g_rgb_data, 3, S2MU106_RGB_MODE_CONST);
		s2mu106_rgb_set_curr(g_rgb_data, 3, 100);
