...
// Copyright (C) 2021 E-ROUM Co., Ltd. Terry Choi <dw.choi@e-roum.com>
...
#if defined(USE_PWM)
static void lm3692x_pwm_ctrl(struct lm3692x_led *led, int on)
{
	led->pwm_en = on;
	if(on) {
		if(pinctrl_select_state(led->pinctrl, led->pwm_on)) {
			dev_err(&led->client->dev, "failed to turn on pwm\n");
		} else {
			dev_dbg(&led->client->dev, "pinctrl select pwm on state success\n");
			pwm_enable(led->pwm);
		}
	} else {
		if(pinctrl_select_state(led->pinctrl, led->pwm_off)) {
			dev_err(&led->client->dev, "failed to turn off pwm\n");
		} else {
			dev_dbg(&led->client->dev, "pinctrl select pwm off state success\n");
			pwm_disable(led->pwm);
		}
	}
}
...
#if defined(USE_PWM)
	ret = regmap_write(led->regmap, LM3692X_BRT_CTRL, LM3692X_BRHT_MODE_MULTI_RAMP);
#endif
	if (ret)
		goto out;
...
static int lm3692x_probe_dt(struct lm3692x_led *led)
{
	...
	led->asel_gpio = devm_gpiod_get_optional(&led->client->dev,
						   "asel", GPIOD_OUT_LOW);
	if (IS_ERR(led->asel_gpio)) {
		ret = PTR_ERR(led->asel_gpio);
		dev_err(&led->client->dev, "Failed to get asel gpio: %d\n",
			ret);
		return ret;
	}
	gpiod_set_value(led->asel_gpio, 0); //I2C Address -> 36h
	...
	if (IS_ERR(led->regulator)) {
		...
		if (ret != -ENODEV) {
			dev_err(&led->client->dev, "Failed to get vled regulator %d\n", ret);
			return ret;
		}
	...
	//Default 10kHz
	led->pwm_duty = 100000;
	led->pwm_period = 100000;

	ret = of_property_read_u32(led->client->dev.of_node, "pwm_id", &pwm_id);
	if (ret < 0) {
		dev_err(&led->client->dev, "error to get dt node pwm_id %d\n", pwm_id);
		led->pwm = NULL;
	} else {
		led->pinctrl = devm_pinctrl_get(&led->client->dev);
		if (IS_ERR(led->pinctrl)) {
			dev_err(&led->client->dev, "failed to get pwm-%d pinctrl\n", pwm_id);
		} else {
			led->pwm_on = pinctrl_lookup_state(led->pinctrl, "pwm_on");
			led->pwm_off = pinctrl_lookup_state(led->pinctrl, "pwm_off");

			if (!IS_ERR(led->pwm_on) && !IS_ERR(led->pwm_off)) {
				led->pwm = pwm_request(pwm_id, "lm36922_pwm");
				if(IS_ERR(led->pwm)) {
					ret = PTR_ERR(led->pwm);
					dev_err(&led->client->dev, "unable to request PWM for LED, error: %d\n", ret);
					led->pwm = NULL;
				}
			} else {
				dev_err(&led->client->dev, "failed to get pwm_on or pwm_off pin state\n");
			}
		}
	}
#endif
	...
	led->boost_ctrl = /*LM3692X_BOOST_SW_1MHZ |*/
		...
		LM3692X_OCP_PROT_1_5A | LM3692X_MIN_IND_22UH;
	...
	led->led_dev.name = dev_name(&led->client->dev);
...
#if defined(USE_PWM)
static ssize_t lm3692x_show_pwm_config(struct device *dev,
			    struct device_attribute *attr,
			    char *buf)
{
	struct lm3692x_led *led = dev_get_drvdata(dev);
...
static ssize_t lm3692x_store_pwm_config(struct device *dev,
			     struct device_attribute *attr,
			     const char *buf, size_t len)
{
	struct lm3692x_led *led = dev_get_drvdata(dev);

	sscanf(&buf[0], "%d %d", &led->pwm_duty, &led->pwm_period);
	if(led->pwm) {
		pwm_config(led->pwm, led->pwm_duty, led->pwm_period);
	}
...
static DEVICE_ATTR(pwm_cfg, S_IRUGO | S_IWUSR, lm3692x_show_pwm_config, lm3692x_store_pwm_config);

static ssize_t lm3692x_show_pwm_enable(struct device *dev,
			    struct device_attribute *attr,
			    char *buf)
{
	struct lm3692x_led *led = dev_get_drvdata(dev);
...
static ssize_t lm3692x_store_pwm_enable(struct device *dev,
			     struct device_attribute *attr,
			     const char *buf, size_t len)
{
	struct lm3692x_led *led = dev_get_drvdata(dev);
	...
	if(led->pwm) {
		lm3692x_pwm_ctrl(led, !!enable);
	}

	return len;
}
static DEVICE_ATTR(pwm_en, S_IRUGO | S_IWUSR, lm3692x_show_pwm_enable, lm3692x_store_pwm_enable);
#endif

static struct attribute *lm3692x_attrs[] = {
#if defined(USE_PWM)
	&dev_attr_pwm_cfg.attr,
	&dev_attr_pwm_en.attr,
#endif
	NULL,
};

static struct attribute_group lm3692x_attr_group = {
	.attrs = lm3692x_attrs,
};
			...
			const struct i2c_device_id *id)
{
	...
	ret = sysfs_create_group(&client->dev.kobj, &lm3692x_attr_group);
	if (ret) {
