...
static int s3c24xx_serial_probe(struct platform_device *pdev)
{
	...
	pinctrl_irq = devm_pinctrl_get_select(&pdev->dev, "default");
	if (IS_ERR(pinctrl_irq))
			dev_err(&pdev->dev, " Failed to configure irq pin\n");
...
static int s3c24xx_serial_suspend(struct device *dev)
{
	...
	pinctrl_irq = devm_pinctrl_get_select(dev, "sleep");
	if (IS_ERR(pinctrl_irq))
...
static int s3c24xx_serial_resume(struct device *dev)
{
	...
	pinctrl_irq = devm_pinctrl_get_select(dev, "default");
	if (IS_ERR(pinctrl_irq))
							dev_err(dev, " Failed to configure irq pin\n");
	return 0;
}
