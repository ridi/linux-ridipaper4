...
static irqreturn_t s2mu107_sc_fault_isr(int irq, void *data)
{
	...
	if (fault == CHG_STATUS_WD_SUSPEND || fault == CHG_STATUS_WD_RST) {
		psy = power_supply_get_by_name("s2mu107-usbpd");
		if (!psy)
			pr_err("%s, fail to set topoff current to FG\n", __func__);
		else {
			value.intval = 1;
			pr_info("%s, reset USBPD\n", __func__);
			power_supply_set_property(psy, POWER_SUPPLY_PROP_USBPD_RESET, &value);
		}
	}
...
static irqreturn_t s2mu107_sc_sys_isr(int irq, void *data)
{
	struct s2mu107_sc_data *sw_charger = data;
	u8 val;

	s2mu107_read_reg(sw_charger->i2c, S2MU107_SC_STATUS4, &val);
	pr_info("%s : STATUS4 %02x\n", __func__, val);
...
static int s2mu107_switching_charger_probe(struct platform_device *pdev)
{
	 ...
	 */
	sw_charger->irq_sys = pdata->irq_base + S2MU107_SC_IRQ1_SYS;
	ret = request_threaded_irq(sw_charger->irq_sys, NULL,
			s2mu107_sc_sys_isr, 0, "sys-irq", sw_charger);
	if (ret < 0) {
		dev_err(s2mu107->dev, "%s: Fail to request SYS in IRQ: %d: %d\n",
				__func__, sw_charger->irq_sys, ret);
		goto err_reg_irq;
	}
