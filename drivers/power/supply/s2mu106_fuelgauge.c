...
#define OFF_POLL_DELAY_S   (20 * 60)  /* 20 minutes RTC wait time */
#define AUTO_DISCHARGE_BATTERY_VOLT_START (3830) /* as determined by EE team */
...
static void s2mu106_init_regs(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	/* Force VNTC on always (rather than cycling on/off)? */
	if (fuelgauge->pdata->force_vntc) {
		s2mu106_read_reg_byte(fuelgauge->i2c, S2MU106_REG_RESET, &temp);
		temp = temp | S2MU106_REG_RESET_VNTC_ON;
...
static int get_battery_temperature(int *temp)
{
	struct power_supply *psy;
	union power_supply_propval value;
	int ret;
	...
	if (ret < 0) {
		pr_err("%s: Fail to execute properly\n", __func__);
		return ret;
	}
...
static void s2mu106_temperature_compensation(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	pr_debug("%s: diff_soc = %d, data[1] = 0x%02x, data[0] = 0x%02x\n",
			__func__, diff_soc, data[1], data[0]);
}
#endif
...
static int s2mu106_get_rawsoc(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	/* Garmin added setting of VNTC_ON bits, so need to clear them for
	   Samsung code looking at the rest of this register contents */
	por_state &= ~(S2MU106_REG_RESET_VNTC_ON);
	...
	curr = s2mu106_get_current(fuelgauge) / 1000;

	fuelgauge->temperature = 250; /* initialize to default in case of failure */
	ret = get_battery_temperature(&fuelgauge->temperature);
	if (ret) {
		pr_err("getting temperature from battery failed\n");
	}
...
static int s2mu106_fuelgauge_parse_dt(struct s2mu106_fuelgauge_data *fuelgauge)
{
	...
	if (np == NULL) {
		...
		if (ret < 0) {
			...
			pr_warn("%s Default value of fuel_alert_vol : %d\n",
		...
		ret = of_property_read_u32(np, "fuelgauge,fuel_alert_soc",
				&fuelgauge->pdata->fuel_alert_soc);
		if (ret < 0)
			pr_warn("%s error reading pdata->fuel_alert_soc %d\n",
					__func__, ret);

		fuelgauge->pdata->force_vntc = of_property_read_bool(np,
		...
		if (!np) {
			...
			if (ret < 0)
				pr_warn("%s error reading battery,fuelgauge_name\n", __func__);
		}
		...
		if (!np) {
			...
			if (ret < 0) {
				...
				pr_warn("%s There is no cell1 accumulative rate in DT. Use default value(0x800)\n", __func__);
			}

			ret = of_property_read_u32(np, "battery,battery_param_ver",
					&fuelgauge->info.battery_param_ver);
			if (ret < 0)
				pr_warn("%s There is no battery parameter version\n", __func__);

			ret = of_property_read_u32(np, "battery,low_temp_limit",
					&fuelgauge->low_temp_limit);
			if (ret < 0) {
				pr_warn("%s There is no low temperature limit. Use default(100)\n",
						__func__);
...
static void s2mu106_fuelgauge_shutdown(struct i2c_client *client)
{
	...
	/* Turn off VNTC to reduce power consumption while off */
	if (fuelgauge->pdata->force_vntc) {
		s2mu106_read_reg_byte(fuelgauge->i2c, S2MU106_REG_RESET, &temp);
		temp &= ~S2MU106_REG_RESET_VNTC_MASK;
		temp |= S2MU106_REG_RESET_VNTC_OFF;
		s2mu106_write_and_verify_reg_byte(fuelgauge->i2c,
						  S2MU106_REG_RESET,
						  temp);
	}
	...
	dev_info(&client->dev, "battery voltage : %d", vbat);
#if defined(CONFIG_MACH_RP400)
	alarm.enabled = 0;
	alarm.pending = 0;
	ret = rtc_set_boot_alarm(rtc, &alarm);
	dev_err(&client->dev, "clear RTC alarm during shutdown: %d", ret);
#else
	if (vbat > AUTO_DISCHARGE_BATTERY_VOLT_START) {
		ret = rtc_read_time(rtc, &alarm.time);
		if (ret < 0) {
			dev_err(&client->dev,
				"Failed to read RTC during powerdown: %d", ret);
			return;
		}

		rtc_tm_to_time(&alarm.time, &now);
		later = now + OFF_POLL_DELAY_S;
		rtc_time_to_tm(later, &alarm.time);

		alarm.enabled = 1;
		ret = rtc_set_boot_alarm(rtc, &alarm);
		if (ret < 0) {
			dev_err(&client->dev,
			   "Failed to set RTC alarm during shutdown: %d", ret);
			return;
		}
		dev_info(&client->dev, "RTC alarm set succesfully for %d secs", OFF_POLL_DELAY_S);
	} else {
		alarm.enabled = 0;
		alarm.pending = 0;
		ret = rtc_set_boot_alarm(rtc, &alarm);
		if (ret < 0)
			dev_err(&client->dev,
			   "Failed to clear RTC alarm during shutdown: %d", ret);
	}
#endif
}
...
static int s2mu106_fuelgauge_remove(struct i2c_client *client)
{
	...
	s2mu106_fuelgauge_shutdown(client);
