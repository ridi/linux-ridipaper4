...
#define ONE_MINUTE_IN_NS   60UL * 1000UL * 1000UL * 1000UL

#define POWEROFF_CHARING			1
#define POWEROFF_FULL				4


extern void poweroff_full_led(void);
extern void poweroff_notcharging_led(void);
extern void poweroff_charging_led(void);

static int get_thermistor_temperature(struct s2m_chg_manager_info *battery)
{
	int ret;
	...
	ret = thermal_zone_get_temp(battery->tzd_batt, &battery->temperature);
	if (ret)
		return ret;

	/* temperature is set in milli-degrees Celsius, but this driver
	 * uses temperatures in deci-degrees Celsius */
	battery->temperature /= 100;
	pr_debug("%s: T = %d\n", __func__, battery->temperature);
	return 0;
}

static int get_fuelgauge_temperature(struct s2m_chg_manager_info *battery)
{
...
static int s2m_update_temperature(struct s2m_chg_manager_info *battery)
{
	/* Get temperature from the thermal zone device if available. If not,
	 * fall back to using the fuelgauge for temperature reading */
	battery->temperature = 250; /* default value in case of a failure. */
	if (battery->tzd_batt) {
		/* There is a known issue with s2mu106's reported
		 * temperature being unstable during the first few
		 * seconds after VNTC is turned ON in this driver.
		 * Use the thermistor temperature initially. */
		if (ktime_get_boot_fast_ns() < ONE_MINUTE_IN_NS) {
			if (get_thermistor_temperature(battery))
				pr_err("%s: Failed to get battery temperature\n", __func__);
		} else {
			get_fuelgauge_temperature(battery);
		}
	} else {
		/* Fall back to using the fuelgauge for temperature reading */
		pr_debug("get temperature from fuelgauge because thermal zone is not set\n");
		get_fuelgauge_temperature(battery);
	}
	return 0;
}
...
#if defined(CONFIG_MACH_RP400) //dw.choi
extern int get_epd_usb_role(void);
extern int dsim_status(void);
extern void send_wakeup_key_event(void);
extern void set_rp400_usb_detect(int detect);
extern int get_rp400_usb_lock(void);

static u32 g_default_input_current;
int battery_capacity = -1;
int battery_status = -1;
struct s2m_chg_manager_info *g_battery;
#endif

static int s2m_property_is_writeable(struct power_supply *psy,
...
static void get_charging_current(struct s2m_chg_manager_info *battery,
				 ...
				 int *topoff_curr, int wcin_online, int wcin_input_current)
{
	...
	if (wcin_online) {
		/* WCIN is online */
		charging_current = battery->pdata->wcin_charging_current;
		topoff_current = battery->pdata->wcin_full_check_current;
		pr_info("%s: cable_type(%d), WCIN current(%d, %d, %d)\n", __func__,
			battery->cable_type, wcin_input_current,
			charging_current, topoff_current);
	} else {
	/* If WCIN not online */
	pr_info("%s: cable_type(%d), CHGIN current(%d, %d, %d)\n", __func__,
		battery->cable_type, input_current, charging_current,
		topoff_current);

#if defined(CONFIG_MACH_RP400)
	if(dsim_status() == 5) {
		input_current = g_default_input_current;
		charging_current = g_default_input_current;
	}
#else
...
static int set_charging_current(struct s2m_chg_manager_info *battery, int coeff)
{
...
#endif
	union power_supply_propval wcin_input_current;
	union power_supply_propval wcin_online;
	struct power_supply *wcin_psy = power_supply_get_by_name(battery->pdata->wcin_name);
	wcin_input_current.intval = 0;
	wcin_online.intval = 0;

	if (wcin_psy) {
		if (power_supply_get_property(wcin_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &wcin_input_current) < 0) {
			pr_err("%s: Can't read WCIN input current", __func__);
		}
		if (power_supply_get_property(wcin_psy, POWER_SUPPLY_PROP_ONLINE, &wcin_online) < 0) {
			pr_err("%s: Can't read WCIN online status", __func__);
		}
	}
	...
	get_charging_current(battery, &input_current,
			     &charging_current, &topoff_current,
			     wcin_online.intval, wcin_input_current.intval);
	...
	if (wcin_online.intval) {
		pr_info("%s: WCIN input curr. (%d), charge curr. (%d -> %d), "
			"top-off curr. (%d -> %d)\n",	__func__,
			wcin_input_current.intval,
			battery->charging_current, charging_current,
			battery->topoff_current, topoff_current);
	} else {
		pr_info("%s: CHGIN input curr. (%d -> %d), charge curr. (%d -> %d), "
			"top-off curr. (%d -> %d)\n",	__func__,
	...
	pr_err("%s: CHGIN input current (%d -> %d)\n", __func__, battery->input_current, input_current);
...
static int set_battery_status(struct s2m_chg_manager_info *battery, int status)
{
	...
	pr_err("%s: current status = %d, new status = %d\n",
...
#if defined(CONFIG_MACH_RP400)
int set_rp400_battery_status(struct s2m_chg_manager_info *battery, int status)
{
	return set_battery_status(battery, status);
}

int set_rp400_charging_current(struct s2m_chg_manager_info *battery, int max, int now)
{
	struct power_supply *psy;
	union power_supply_propval value;
	...
	battery->input_current = max;
	battery->charging_current = now;

	pr_err("%s (%d, %d)\n", __func__, battery->input_current, battery->charging_current);

	value.intval = battery->input_current;
	power_supply_set_property(psy, POWER_SUPPLY_PROP_CURRENT_MAX, &value);
	value.intval = battery->charging_current;
	power_supply_set_property(psy, POWER_SUPPLY_PROP_CURRENT_NOW, &value);
...
{
#if defined(CONFIG_MACH_RP400)
	struct power_supply *psy_pm = power_supply_get_by_name("s2mu106_pmeter");
	union power_supply_propval val;

	power_supply_get_property(psy_pm, POWER_SUPPLY_PROP_VCHGIN, &val);
	pr_err("%s cable_type: %d, VCHGIN: %d\n", __func__, battery->cable_type, val.intval);
	...
	if (battery->pdata->wcin_name)
		psy = power_supply_get_by_name(battery->pdata->wcin_name);
	if (psy) {
		ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_ONLINE,
						&value);
		if (!ret)
			wcin_present = value.intval;
	}
	/* If no WCIN (i.e. any charging must be from USB cable), then
	   use the cable type to determine if we can charge */
	if (!wcin_present) {
		if (battery->cable_type == POWER_SUPPLY_TYPE_BATTERY ||
		    battery->cable_type == POWER_SUPPLY_TYPE_UNKNOWN ||
		    battery->cable_type == POWER_SUPPLY_TYPE_OTG) {
	...
	dev_err(battery->dev, "%s: abnormal cable_type or status", __func__);
#endif
}
...
static int s2m_ifconn_handle_wcin_notification(struct notifier_block *nb,
					      unsigned long action, void *data)
{
	struct s2m_chg_manager_info *battery = container_of(nb,
							    struct s2m_chg_manager_info,
	...
	/* Just confirm right action, then re-check battery charger enable */
	if (action == IFCONN_NOTIFY_ID_WCIN)
		set_bat_status_by_cable(battery);
	else
...
static int get_battery_info(struct s2m_chg_manager_info *battery)
{
	...
	value.intval = S2M_BATTERY_CURRENT_UA;
	...
	s2m_update_temperature(battery);
	...
	dev_info(battery->dev,
		 ...
		 battery->voltage_now, battery->current_now / 1000,
	...
	dev_dbg(battery->dev,
		...
		battery->voltage_avg, battery->voltage_ocv, battery->current_avg / 1000);
#if defined(CONFIG_MACH_RP400)
	battery_capacity = battery->capacity;
	battery_status = battery->status;
	//pr_err("battery_capacity %d%%\n", battery_capacity);
	//pr_err("battery_status %d\n", battery_status);
	if(battery_capacity <= 3) {
		set_rp400_usb_detect(3);
		send_wakeup_key_event();
	}/* else if(battery_status == POWER_SUPPLY_STATUS_FULL) {
		set_rp400_usb_detect(4);
	}*/
#endif
...
static void check_charging_full(struct s2m_chg_manager_info *battery)
{
	...
	union power_supply_propval wcin_online;
	int topoff_current = battery->pdata->charging_current[battery->cable_type].full_check_current;
	struct power_supply *psy = power_supply_get_by_name(battery->pdata->wcin_name);

	pr_debug("%s Start\n", __func__);

	ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_ONLINE, &wcin_online);
	if (ret < 0) {
		pr_err("%s: Fail to get wcin online status: %d", __func__, ret);
	} else if (wcin_online.intval) {
		/* WCIN is online */
		topoff_current = battery->pdata->wcin_full_check_current;
	}
	...
	if ((battery->current_now >= 0 &&
	     battery->current_now < topoff_current * 1000) &&
	...
	if (battery->full_check_cnt >= battery->pdata->full_check_count) {
		...
		pr_err("%s: Full charged, charger off\n", __func__);
	}
}
...
#if defined(CONFIG_MACH_RP400)
	if((status != battery->status) || (health != battery->health) || (capacity != battery->capacity)) {
		power_supply_changed(battery->psy_battery);
...
continue_monitor:
#if defined(CONFIG_MACH_RP400)
	if((status != battery->status) || (health != battery->health) || (cable_type != battery->cable_type)) {
		pr_err("%s: Status(%s), Health(%s), Cable(%d), Recharging(%d))\n", __func__,
			bat_status_str[battery->status],
			health_str[battery->health],
			battery->cable_type,
				    ...
				    struct s2m_chg_manager_info *battery)
{
	...
	ret = of_property_read_string(np, "battery,wcin_name",
				      (char const **)&pdata->wcin_name);
	if (ret)
		pr_info("%s: WCIN name is empty\n", __func__);
...
#if defined(CONFIG_MACH_RP400)
	g_default_input_current = default_input_current;
#endif
	...
	/* A missing thermal zone is not an error, don't capture ret,
	 * the driver will fallback to use the fuelgauge temperature */
	of_property_read_string(np, "battery,thermal_zone",
			(char const **)&pdata->thermal_zone_name);
	if (pdata->thermal_zone_name)
		pr_info("%s: battery thermal zone name: %s\n",
				__func__, pdata->thermal_zone_name);

	ret = of_property_read_u32(np, "battery,wcin_charging_current",
				   &pdata->wcin_charging_current);
	if (ret)
		pr_warn("%s: battery,wcin_charging_current not set", __func__);

	ret = of_property_read_u32(np, "battery,wcin_full_check_current",
				   &pdata->wcin_full_check_current);
	if (ret)
		pr_warn("%s: battery,wcin_full_check_current not set", __func__);
...
static int s2m_chg_manager_probe(struct platform_device *pdev)
{
	...
	battery->psy_battery_desc.property_is_writeable = s2m_property_is_writeable;
	...
	battery->tzd_batt = thermal_zone_get_zone_by_name(battery->pdata->thermal_zone_name);
	if (IS_ERR(battery->tzd_batt))  {
		dev_info(&pdev->dev, "did not find battery thermal zone: %s\n",
				battery->pdata->thermal_zone_name);
		battery->tzd_batt = NULL;
	} else {
		dev_info(&pdev->dev, "found battery thermal zone: %s\n",
				battery->pdata->thermal_zone_name);
	}
	...
	ifconn_notifier_register(&battery->ifconn_cc_nb,
				 ...
				 IFCONN_NOTIFY_CCIC);
#endif
	ifconn_notifier_register(&battery->ifconn_wcin_nb,
				 s2m_ifconn_handle_wcin_notification,
				 IFCONN_NOTIFY_BATTERY,
				 IFCONN_NOTIFY_CHARGER);
#endif
...
{
	ifconn_notifier_unregister(IFCONN_NOTIFY_CHARGER, IFCONN_NOTIFY_BATTERY);
	return 0;
}
