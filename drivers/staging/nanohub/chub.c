...
static int contexthub_ipc_probe(struct platform_device *pdev)
{
	...
	chub->pinctrl = devm_pinctrl_get(chub->dev);
	if (IS_ERR(chub->pinctrl)) {
	    pr_err("%s: failed to get pinctrl \n", __func__);
	    chub->pinctrl = NULL;
	    goto err;
	}
	chub->spi_suspend = pinctrl_lookup_state(chub->pinctrl, "spi_suspend");
	if (IS_ERR(chub->spi_suspend)) {
	    pr_err("%s: failed to lookup spi_suspend state \n", __func__);
	    chub->spi_suspend = NULL;
	    goto err;
	}
	chub->spi_resume = pinctrl_lookup_state(chub->pinctrl, "spi_resume");
	if (IS_ERR(chub->spi_resume)) {
	    pr_err("%s: failed to lookup spi_resume state \n", __func__);
	    chub->spi_resume = NULL;
	    goto err;
	}
	chub->i2c_suspend = pinctrl_lookup_state(chub->pinctrl, "i2c_suspend");
	if (IS_ERR(chub->i2c_suspend)) {
	    pr_err("%s: failed to lookup i2c_suspend state \n", __func__);
	    chub->i2c_suspend = NULL;
	    goto err;
	}
	chub->i2c_resume = pinctrl_lookup_state(chub->pinctrl, "i2c_resume");
	if (IS_ERR(chub->i2c_resume)) {
	    pr_err("%s: failed to lookup i2c_resume state \n", __func__);
	    chub->i2c_resume = NULL;
	    goto err;
	}
	if (chub->pinctrl != NULL) {
	    pinctrl_select_state(chub->pinctrl, chub->spi_suspend);
	    pinctrl_select_state(chub->pinctrl, chub->spi_resume);
	    pinctrl_select_state(chub->pinctrl, chub->i2c_suspend);
	    pinctrl_select_state(chub->pinctrl, chub->i2c_resume);
	}
...
static int contexthub_suspend(struct device *dev)
{
	...
	ipc_hw_gen_interrupt(AP, IRQ_EVT_CHUB_ALIVE);
    if (ipc->pinctrl != NULL) {
        pinctrl_select_state(ipc->pinctrl, ipc->i2c_suspend);
        pinctrl_select_state(ipc->pinctrl, ipc->spi_suspend);
    }
#ifdef CONFIG_CHRE_SENSORHUB_HAL
...
static int contexthub_resume(struct device *dev)
{
	...
	ipc_hw_gen_interrupt(AP, IRQ_EVT_CHUB_ALIVE);
    if (ipc->pinctrl != NULL) {
        pinctrl_select_state(ipc->pinctrl, ipc->spi_resume);
        pinctrl_select_state(ipc->pinctrl, ipc->i2c_resume);
    }
#ifdef CONFIG_CHRE_SENSORHUB_HAL
