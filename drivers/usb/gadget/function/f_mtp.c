...
#if defined(CONFIG_MACH_RP400)
int is_operating_mtp(void)
{
	return (_mtp_dev)?(_mtp_dev->operating_mtp):0;
}
#endif
...
#if defined(CONFIG_MACH_RP400)
			schedule_delayed_work(&dev->epd_work, msecs_to_jiffies(0));
#endif
			return -EINVAL;
		}
	}
...
#if defined(CONFIG_MACH_RP400)
	schedule_delayed_work(&dev->epd_work, msecs_to_jiffies(500));
#endif
	spin_unlock_irq(&dev->lock);
...
#if defined(CONFIG_MACH_RP400)
static void mtp_epd_work(struct work_struct *work)
{
	struct mtp_dev *dev = container_of(work, struct mtp_dev, epd_work.work);
	dev->operating_mtp = 0;
}
#endif
...
#if defined(CONFIG_MACH_RP400)
	if(!_mtp_dev->epd_work_en) {
		INIT_DELAYED_WORK(&_mtp_dev->epd_work, mtp_epd_work);
		_mtp_dev->epd_work_en = 1;
	}
#endif
