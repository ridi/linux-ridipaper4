...
static void aud3004x_amp_ldo_on(struct aud3004x_priv *aud3004x)
{
	...
	aud3004x->amp_ldo_gpio = of_get_named_gpio(np, "amp-ldo-gpio", 0);
         if (!gpio_is_valid(aud3004x->amp_ldo_gpio)) {
                 dev_err(dev, "%s failed to get a amp_ldo_gpio\n", __func__);
         } else {
                 gpio_request(aud3004x->amp_ldo_gpio, "amp-ldo-gpio");
                 dev_dbg(dev, "ON: amp_ldo_gpio is %d", aud3004x->amp_ldo_gpio);
                 gpio_direction_output(aud3004x->amp_ldo_gpio, 1);
                 gpio_free(aud3004x->amp_ldo_gpio);
	}
}

static void aud3004x_amp_ldo_off(struct aud3004x_priv *aud3004x)
{
	struct device *dev = aud3004x->dev;
	struct device_node *np = dev->of_node;
	unsigned int feature_flag;
	...
	aud3004x->amp_ldo_gpio = of_get_named_gpio(np, "amp-ldo-gpio", 0);
         if (!gpio_is_valid(aud3004x->amp_ldo_gpio)) {
                 ...
                 gpio_request(aud3004x->amp_ldo_gpio, "amp-ldo-gpio");
                 dev_dbg(dev, "OFF: amp_ldo_gpio is %d", aud3004x->amp_ldo_gpio);
                 gpio_direction_output(aud3004x->amp_ldo_gpio, 0);
                 gpio_free(aud3004x->amp_ldo_gpio);
	}
}
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		/* Turn on amplifier ldo */
		aud3004x_amp_ldo_on(aud3004x);
		mdelay(200);
		dev_dbg(codec->dev, "Amplifier ldo is on\n");
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		break;
	case SND_SOC_DAPM_PRE_PMD:
		/* Turn off amplifier ldo */
		aud3004x_amp_ldo_off(aud3004x);
		mdelay(200);
		dev_dbg(codec->dev, "Amplifier ldo is off\n");
