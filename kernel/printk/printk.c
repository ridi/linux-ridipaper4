...
extern int chg_kernel_log_format;
#endif
...
static size_t print_time(u64 ts, char *buf)
{
	...
	if(chg_kernel_log_format) {
		...
		if (!buf)
			return snprintf(NULL, 0, "[%02d-%02d %02d:%02d:%02d.%03ld] [%5lu.000000] ",
				tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, time.tv_usec/1000, (unsigned long)ts);

		return sprintf(buf, "[%02d-%02d %02d:%02d:%02d.%03ld] [%5lu.%06lu] ",
			tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, time.tv_usec/1000, (unsigned long)ts, rem_nsec/1000);
	}
#endif
