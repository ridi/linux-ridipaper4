...
#define SEL_PRIO_WCIN_MASK		BIT(SEL_PRIO_WCIN_SHIFT)
...
typedef struct s2mu106_charger_platform_data {
	...
	int wcin_current_limit;
	enum power_supply_type wcin_type;
	bool wcin_priority;
	bool use_batt_temp_missing;
	int batt_temp_missing;
	int open_ntc_margin;
...
struct s2mu106_charger_data {
	...
	struct power_supply_desc psy_wc_desc;
	...
	struct work_struct notify_chg_manager_work;
	struct delayed_work reset_wcin_prio_work;
};
